...
impl DesireVariables {
    fn do_assumed_variables_exist(&self) -> RoiExists {
        let actual_roi_var_exists = self.actual_reward.is_some() && self.actual_difficulty.is_some();
        let assumed_roi_var_exists = self.assumed_reward.is_some() && self.assumed_difficulty.is_some();
    ...
    pub fn get_roi(&self) -> Option<u128> {
        match self.do_assumed_variables_exist() {
            ...
            RoiExists::BothAssumedAndActualRoiVarExist => {
                Some(((self.assumed_reward.unwrap() - self.assumed_difficulty.unwrap())
                    / (self.actual_reward.unwrap() - self.actual_difficulty.unwrap())) as u128)
            }
            // todo - logging the issue would be useful
            RoiExists::NoRoiVarExist => {
                None
            }
        }
    }
